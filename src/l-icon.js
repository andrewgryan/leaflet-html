...
class LIcon extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Strings
    let keys = ["icon-url", "icon-retina-url", "shadow-url", "shadow-retina-url", "class-name"]
    keys.forEach(key => {
    ...
    // Points
    let points = ["icon-anchor", "icon-size", "shadow-anchor", "shadow-size", "tooltip-anchor", "popup-anchor"]
    points.forEach(key => {
      if (this.hasAttribute(key)) {
